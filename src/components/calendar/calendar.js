...
var calendar = {
  ...
  solar2lunar: function solar2lunar(yPara, mPara, dPara) {
    ...
    var solarDate = dayjs().year(y).month(m - 1).date(d).format('YYYY-MM-DD');
    var lunarDate = dayjs().year(year).month(month - 1).date(day).format('YYYY-MM-DD');
    ...
    return {
      ...
      'IDayCn': (year > 2100 || year < 1900) ? '' : this.toChinaDay(day),
      'cYear': y || '',
      'cMonth': m || '',
      'cDay': d || '',
      'gzYear': gzY,
